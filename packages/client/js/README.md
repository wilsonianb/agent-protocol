...
```bash
node_modules/.bin/openapi --input ../schemas/openapi.yml --output ./src --name AgentProtocol
```
...
cd agent-protocol/packages/client/js
npm install
npx openapi --input ../../schemas/openapi.yml --output ./src --name AgentProtocol
npm run generate
npm run build
```
